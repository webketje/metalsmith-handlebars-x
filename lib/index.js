...
function partialName(partialsDir, partialPath) {
  ...
  return normalizedPath(
    partialPath.slice(normalized.length + 1, partialPath.lastIndexOf('.'))
  );
}
...
module.exports = function (options) {
  ...
  return function handlebarsX(fileList, metalsmith) {
    ...
    const allPartials = multimatch(
      fileNames,
      `**/${config.partials}/**/*.{hbs,handlebars}`
    );
    const globalPartials = multimatch(
      allPartials,
      `${config.partials}/**/*.{hbs,handlebars}`
    );
    ...
    Object.keys(templateDirs).forEach(function (basedir, i) {
      ...
      templateDirs[basedir].forEach(function (filePath) {
        ...
        try {
          ...
          debug(
            "Error: Handlebars compile - %s for '%s'",
            err.message,
            filePath
          );
        }
